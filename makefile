...
ARCHLDSCRIPT	:= -T"A:/SystemSoftware/University_Program/Monitor_Program/build/altera-socfpga-hosted.ld"
ASFLAGS		:= $(ARCHASFLAGS)
...
# Targets
COMPILE: FinalProject.srec

FinalProject.srec: FinalProject.axf
	$(RM) $@
	$(OC) $(OCFLAGS) $< $@
	$(OD) -S $< > $<.objdump

FinalProject.axf: $(OBJS)
	$(RM) $@
	$(CC) $(LDFLAGS) $(OBJS) -o $@
...
CLEAN: 
	$(RM) FinalProject.srec FinalProject.axf $(OBJS)
